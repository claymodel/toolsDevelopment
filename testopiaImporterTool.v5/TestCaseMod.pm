...
#				  Update 12-April-2011 @hasnat 

package Testopia::TestCaseMod;
...
sub _check_cases {
    ...
    foreach my $caseid (split(/[\s,]+/, $caseids)){
        ...
        push @cases, Testopia::TestCaseMod->new($caseid);
    }
...
#hasnat
sub set_action             { $_[0]->set('action',    $_[1].','.$_[2].','.$_[3]); }
sub set_effect             { $_[0]->set('effect',    $_[1].','.$_[2].','.$_[3]); }
sub set_setup              { $_[0]->set('setup',     $_[1].','.$_[2].','.$_[3]); }
sub set_breakdown          { $_[0]->set('breakdown', $_[1].','.$_[2].','.$_[3]); }
...
sub create {
    ...
    my $col_Y    = $field_values->{col_Y};
    my $col_Z    = $field_values->{col_Z};
    my $caseid_edit    = $field_values->{caseid_edit};
    ...
    # calling _create_data.
    foreach my $field (qw(action effect col_Y col_Z caseid_edit setup breakdown dependson blocks plans runs components tags bugs)){
        delete $field_values->{$field};
    }
    
    my $self = $class->SUPER::insert_create_data($field_values);

    $self->store_text($self->id, $field_values->{'author_id'}, $action, $effect,  $col_Y, $col_Z, $caseid_edit,
                      $setup, $breakdown,0);
...
sub update {
    ...
    $self->store_text_update($self->id,$self->{'author_id'},$self->{'action'}, $self->{'effect'}, 
				$self->{'setup'}, $self->{'breakdown'}, 1,$timestamp);
...
=head2 get_fields_update
#hasnat for REPLACE key word use purposes
=cut

sub get_fields_update {
    my $self = shift;
    my $dbh = Bugzilla->dbh;    
...
sub store {
    ...
    $self->store_text($key, $self->{'author_id'}, $self->{'action'}, $self->{'effect'},  $self->{'col_Y'}, $self->{'col_Z'},$self->{'caseid_edit'},
...
sub store_text {
    my $self = shift;
    my $dbh = Bugzilla->dbh;
    my ($key, $author, $action, $effect, $col_Y, $col_Z, $caseid_edit, $setup, $breakdown, $reset_version, $timestamp) = @_;
    ...
    trick_taint($col_Y) if $col_Y;
    trick_taint($col_Z) if $col_Z;
    trick_taint($caseid_edit) if $caseid_edit;
...
#   check if isValue(caseid_edit) then update
#   otherwise insert
#   Elias Hasnat
#    my $flag = $caseid_edit ? 1 : 0;
#    my $flag2 = class_test_case_db($caseid_edit);
    ...
    if ($is == 1 ){
         ++$version;
         $dbh->do("UPDATE test_case_texts SET creation_ts = now(), case_text_version = $version, who = $author, action = \"$action\", effect = \"$effect\", setup=\"$setup\", breakdown = \"$breakdown\" WHERE case_id = $caseid_edit");
    } else {
        ++$version;
...
#hasnat
sub store_text_update {
    my $self = shift;
    my $dbh = Bugzilla->dbh;
    my ($caseID, $authorID, $action, $effect, $setup, $breakdown, $reset_version, $timestamp) = @_;
    if (!defined $timestamp){
        ($timestamp) = Testopia::Util::get_time_stamp();
    }
    ...
    my $q_action	   = $dbh->quote($action);
    my $q_effect	   = $dbh->quote($effect);
    my $q_breakdown     = $dbh->quote($breakdown);
    my $q_setup	   = $dbh->quote($setup);
    ...
    my $version = $reset_version ? 0 : $self->version || 0;
       ...
       my $ary_ref = $dbh->selectall_arrayref(
		"SELECT action, effect, setup, breakdown, case_text_version 
		FROM test_case_texts 
		WHERE case_id=?",{ Slice => {} },$caseID);
 

        foreach my $ary ( @$ary_ref ) {
	    #print "ACTION : $ary->{action}\n";
	    $action	=	$ary->{action};
	    $effect	=	$ary->{effect};
	    $breakdown	=	$ary->{breakdown};
	    $setup	=	$ary->{setup};
	       my $version = $ary->{case_text_version};
        }
    ...
    my $queryString = "UPDATE test_case_texts SET creation_ts = now(), 
					case_text_version = $version, who = $authorID, 
					action = $q_action, effect = $q_effect, 
					setup=$q_setup, breakdown = $q_breakdown 
					WHERE case_id = $caseID and case_text_version = $version"; 	
	
	# bug fixing Plan 12-4-2011
	# Select by the caseID
	# Replace using the original and new content
	#				
...
sub copy {
    ...
    if ($copydoc){
        $self->store_text($key, Bugzilla->user->id, $self->text->{'action'}, 
                          ...
                          $self->text->{'col_Y'}, $self->text->{'col_Z'}, $self->text->{'caseid_edit'},
...
sub text {
    ...
    my $text = $dbh->selectrow_hashref(
        "SELECT action, effect, col_Y, col_Z, caseid_edit, setup, breakdown, profiles.realname AS author, case_text_version AS version
           FROM test_case_texts
...
sub blocked {
    ...
    foreach my $id (@{$ref}){
        push @deps, Testopia::TestCaseMod->new($id);
    }
    $self->{'blocked'} = \@deps;
    return $self->{'blocked'};
}
