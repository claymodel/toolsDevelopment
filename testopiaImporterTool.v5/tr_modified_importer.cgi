...
if ($action eq 'upload') {
    ...
    if ($type eq 'text/xml'){
        ...
        if ($ctype eq 'json'){
            ...
            $template->process("testopia/import/mod_importer.html.tmpl", $vars) ||
	    ...
	    #$row->{'script'} =~ s/\n/<BR>/g;
	    #$row->{'arguments'}=~ s/\n/<BR>/g;
            #$row->{'default_tester'}=~ s/\n/<BR>/g;
	    #$row->{'sortkey'}=~ s/\n/<BR>/g;
	    #$row->{'summary'}=~ s/\n/<BR>/g;
	    #$row->{'alias'}=~ s/\n/<BR>/g;
	    #$row->{'estimated_time'}=~ s/\n/<BR>/g;
	    #$row->{'dependson'}=~ s/\n/<BR>/g;
	    #$row->{'blocks'}=~ s/\n/<BR>/g;
	    ...
	    $row->{'caseid_edit'} =~ s/\n/<BR>/g;
            ...
            # hasnat
            #print Data::Dumper::Dumper($row);
            my $case = Testopia::TestCaseMod->new({});

	    # $case->hasnat();
	    # print Data::Dumper::Dumper($case->hasnat());
	    ...
	    my $case;	
	    #hasnat
	    # update and new operation selection      
	    
	    my $caseID = $row->{'caseid_edit'} + 0;	
	    #print "(caseID) = $caseID\n";

	    my $updateRequired = Testopia::Util::validate_ID($caseID,'case');

	    #print "updateRequired (caseID)= $updateRequired ($caseID)\n";

	    if($updateRequired == 'undef') { 
			    #print "updateRequired undefined = $updateRequired\n";

			    $case = Testopia::TestCaseMod->create($row);
	    }
	    else { 

			    #print "HASNAT UPDATE";
			    #print Data::Dumper::Dumper($row->{'summary'});
			    #print "updateRequired defined = $updateRequired\n";


			    $case = Testopia::TestCaseMod->new($caseID);	

                	    $case->set_requirement($row->{'requirement'});
  			    $case->set_isautomated($row->{'isautomated'});
			    $case->set_category($row->{'category_id'});
			    
			    # needed to be changed	
			    $case->set_case_status($row->{'case_status_id'});
		           $case->set_priority($row->{'priority_id'});

			    $case->set_script($row->{'script'});

			    $case->set_arguments($row->{'arguments'});
        		    $case->set_default_tester($row->{'default_tester'});
			    $case->set_sortkey($row->{'sortkey'});

			    $case->set_summary($row->{'summary'});

			    $case->set_alias($row->{'alias'});
		          $case->set_estimated_time($row->{'estimated_time'});
			    $case->set_dependson($row->{'dependson'});
			    $case->set_blocks($row->{'blocks'});
				
			    $case->set_action($row->{'action'});
			    $case->set_effect($row->{'effect'});
			    $case->set_setup($row->{'setup'});
			    $case->set_breakdown($row->{'breakdown'});
