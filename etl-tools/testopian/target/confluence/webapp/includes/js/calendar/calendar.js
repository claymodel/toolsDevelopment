...
Date._parseNonDateFormatChars = function(s) {
	...
	//
	var magicChars = "aAbBCdeHIJklmMnpPSstUWVuwyY%";
	var nonMagicChars = "";
	var len = s.length;
	var inMagic = false;
	for (i = 0; i < len; i++) {
...
Date.parseDate = function(str, fmt) {
	...
	for (i = 0; i < a.length; ++i) {
		if (!a[i])
			continue;
		switch (b[i]) {
            //AK - ability to parse time in epoch millis
            // if millis are precedent they take precident over all other options
            case "%r":
                var millis = parseInt(a[i], 10);
                if (!isNaN(millis)){
                    var date = new Date();
                    date.setTime(millis);
                    return date;
...
Date.prototype.print = function (str) {
    ...
    //AK - ability to display time in epoch millis
    s["%r"] = this.getTime();
